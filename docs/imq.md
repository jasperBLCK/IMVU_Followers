...
Клиент шлёт `{"record":"msg_c2g_ping"}` через 15 секунд после последнего
исходящего кадра (`pingInterval = 15*1e3` в `imq.min.js`; таймаут ожидания
сервера — 60 секунд). Без пинга шлюз разрывает соединение и участника
выкидывает из комнаты. Проверено вживую: с пингом 15с соединение держится
в простое без единой реплики; при обрыве нужно заново получить `imq_queue`
из REST, снова зарегистрироваться участником и переподписаться.
